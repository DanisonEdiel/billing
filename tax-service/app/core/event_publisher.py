...
class EventPublisher:
    ...
    async def publish_tax_calculated(self, tax_data: Dict[str, Any], correlation_id: str = None) -> bool:
        """
        Publish a tax calculated event
        """
        event_data = {
            "event_type": "tax_calculated",
            "payload": {
                "tax_amount": tax_data.get("tax_amount"),
                "rules_applied": tax_data.get("rules_applied", []),
                "product_type": tax_data.get("product_type"),
                "original_amount": tax_data.get("original_amount"),
                "calculated_at": datetime.utcnow().isoformat(),
                "user_id": tax_data.get("user_id")
            }
        }
        
        return await self.publish_event("tax.calculated", event_data, correlation_id)
